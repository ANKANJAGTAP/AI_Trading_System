...
- ✅ **#23 Historical option-chain backtests** — ✅ option-leg fill realism + synthetic bid/ask model (`backtest/option_fills.py`); ✅ **NSE bhavcopy historical chains loaded** (`scripts/bhavcopy_backfill.py` → 733 trading days of NIFTY+FINNIFTY, 1.73M strike-rows in the lake; untraded-OHLC coalesced to settlement so the quality gate passes); ✅ **F&O backtest over real chains validated** — NIFTY **+₹390k / 3yr** (50L capital), profitable at *every* spread width (PSR ~0.99), **positive in both forward-OOS halves**, −5% max DD, clean bias audit; PBO 0.71 ⇒ don't tune width; ✅ DhanHQ **live** option-chain vendor (`dhan_chain.py`) for forward greeks/IV. ⬜ remaining: pin/assignment effects; ✅ **FINNIFTY fix backtest-validated** — a min-ATM-OI ≥20k liquidity floor flips it **+₹163k vs −45k** (illiquid strikes were the cost drain); ✅ **live `FnoPipeline` min-OI liquidity gate** added (per-underlying floors; FINNIFTY=20k). **Live paper universe = NIFTY + FINNIFTY** — both forward-OOS validated, sleeve enabled; **BANKNIFTY tested + rejected** (+515k IS → −995k OOS, 16% win, −20% DD — short-premium run over by its trends); SENSEX backfilled via BSE bhavcopy (2024-26, 271k rows) + tested → **rejected** (+64k IS → −807k OOS), parked like BANKNIFTY. So the validated F&O book is **NIFTY + FINNIFTY only**.
...
- ✅ **Phase 5 — Execution/risk** — ✅ options-portfolio Greeks limits, scenario-VaR/stress engine, pin/expiry controls (`risk/greeks_portfolio.py`, `risk/scenario_var.py`, `risk/expiry_control.py`); ✅ composed into `risk/structure_risk.assess_structure`, wired into the paper F&O sim **and now the live `FnoPipeline`** (gate 6b, opt-in `fno.risk_gating`: records net-greeks/stress-VaR/SPAN/expiry on every structure, blocks breaches when enabled) — backtest/live parity. Pin risk is covered (cash-settled indices: no assignment; expiry-day gate + 21-DTE time-exit + the live expiry-block).
- 🟡 **Phase 6 — Compliance + paper-live + UX** — ✅ payoff/Greeks UX + ✅ **structure *builder*** (Structure Lab is now a leg-by-leg composer: editable type/side/strike/lots, add/remove legs, presets seed it, live payoff/greeks/stress-VaR/SPAN via `POST /api/structure/analyze`), ✅ go-live walkthrough (`GO_LIVE.md`). ⬜ remaining: sustained paper across expiry cycles (calendar time, not code).
- ⬜ 🔒 **Phase 7 — Controlled live**: tiny capital, one index/strategy, scale only on evidence. *(user action — real capital.)*
- 🟡 **Data fuel (cross-cutting)** — ✅ **DhanHQ is the data vendor**: `dataplatform/vendors/dhan.py` (daily + 1/5/15/30/60-min REST, `scripts/dhan_backfill.py` → candles, date-chunked + throttled for Dhan's per-call range/rate caps) and `dataplatform/vendors/dhan_chain.py` (live option chain — real greeks/IV/OI/bid-ask via `/optionchain`, `scripts/dhan_chain_snapshot.py`), pure normalizers unit-tested; ✅ **Dhan bars backfilled** — 195,870 real 5-min bars (10 NIFTY large-caps, 2025-06→2026-06); ✅ **NSE bhavcopy chains backfilled** — 3yr NIFTY+FINNIFTY EOD option chains in the lake (733 days, 1.73M rows). *(TrueData removed — superseded by Dhan.)* ✅ **validation tooling** — `scripts/run_sweep.py` (offline equity sweep + gross/net), `scripts/fno_sweep.py` (F&O DSR/PBO over real chains), `scripts/fno_train_meta.py` (meta-label trained through the purged-CV gate — correctly **not** activated: entry features don't beat base rate OOS, so the F&O edge is structural, not filterable). ✅ **F&O ban-list now real** (`NSEBanListProvider` + pure `parse_secban`, daily-cached — guards stock F&O; indices never banned); ⬜ remaining: `strategies/providers.py` *fundamentals* still a stub (needs a paid Indian-fundamentals API; only the stock path uses it).
