...
def _run(args) -> int:
    ...
    def strategy(date, chain, spot):
        ...
        if has_pcr:
            fv["pcr"] = _num(opt_feats["pcr_oi"].get(date))
        captured[date] = fv
        return d.structure
    ...
    with open(out, "w", encoding="utf-8") as fh:
        json.dump({"underlying": args.underlying, "features": feats, "params": final,
                   "metrics": {"accuracy": round(mean_acc, 3), "base_rate": round(mean_base, 3),
                               ...
                               "n_samples": n, "validated": validated, "split": split_desc}},
                  fh, indent=2, default=str)

    print(f"\nCV ({split_desc}):  acc {mean_acc * 100:.0f}%  precision(let-through) {mean_prec * 100:.0f}%  "
          f"lift {mean_lift * 100:+.0f}pp  (base {mean_base * 100:.0f}%)")
